...
const downloadInvoicePdf = (record: PaymentRecord, platformFullName: string) => {
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(13);
  doc.text(platformFullName, marginX + 44, 44);
  ...
  drawParty(marginX + partyWidth + 20, "Billed From", [platformFullName, "support@medleads.ai", "All transactions in USD ($)"]);
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8.5);
  doc.text(`Generated by ${platformFullName}`, marginX, footerTop + 16);
        ...
        { title: "Status", dataIndex: "status", render: renderStatus },
        {
          ...
          render: (_: unknown, record: PaymentRecord) => (
            <Button
              ...
              onClick={() => downloadInvoicePdf(record, platformFullName)}
            >
              Download
            </Button>
          ),
