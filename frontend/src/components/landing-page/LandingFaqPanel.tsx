...
import { landingFaqService, type LandingFaq } from "@/services/landing-faq.service";

interface LandingFaqPanelProps {
  onActionsChange: (node: ReactNode) => void;
}

export function LandingFaqPanel({ onActionsChange }: LandingFaqPanelProps) {
  ...
  const load = async () => {
    try {
      setFaqs(await landingFaqService.list());
    } catch {
  ...
  const openCreate = () => {
    ...
    form.setFieldsValue({ is_active: true, sort_order: nextOrder });
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    try {
      ...
      setSaving(true);
      if (editing) {
        await landingFaqService.update(editing.id, values);
        void message.success("FAQ updated.");
      } else {
        await landingFaqService.create(values);
  ...
  const handleDelete = (faq: LandingFaq) => {
    modal.confirm({
      ...
      onOk: async () => {
        try {
          await landingFaqService.remove(faq.id);
  ...
  const handleToggle = async (faq: LandingFaq, is_active: boolean) => {
    try {
      await landingFaqService.update(faq.id, { is_active });
  ...
  const columns: ColumnsType<LandingFaq> = [
    {
      title: "#",
    ...
    },
    {
      ...
      dataIndex: "is_active",
      width: 90,
      render: (v: boolean, record: LandingFaq) => (
    ...
    },
    {
      ...
      key: "actions",
      width: 100,
      render: (_: unknown, record: LandingFaq) => (
        <Space size={4}>
          <Tooltip title="Edit">
            <Button
              type="text"
    ...
    <div className="mt-6">
      ...
      >
        <Form form={form} layout="vertical" className="mt-4">
          ...
          >
            <Input placeholder="e.g. Will this replace my front-desk staff?" />
          </Form.Item>
          <Form.Item
            name="answer"
            label="Answer"
          ...
          >
            <Input.TextArea
              rows={5}
              placeholder="e.g. No — ClinicFlow handles repetitive tasks so your staff can focus on patients."
            />
          </Form.Item>
          <Form.Item name="sort_order" label="Order" tooltip="Lower numbers appear first on the landing page.">
